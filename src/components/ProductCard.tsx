    ...
    >
      ...
      <Link to={`/product/${product.id}`} className="block">
        <div className="relative aspect-[3/4] overflow-hidden rounded-lg bg-muted product-image-swap">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-2">
            {product.isNew && (
              <span className="bg-sage text-sage-dark text-xs font-medium px-2 py-1 rounded">
                New
              </span>
            )}
            {product.isBestseller && (
              <span className="bg-terracotta text-cream text-xs font-medium px-2 py-1 rounded">
                Bestseller
              </span>
            )}
      ...
      <div className="mt-4">
        <Link to={`/product/${product.id}`}>
          <h3 className="font-medium text-sm hover:text-terracotta transition-colors">
            {product.name}
          </h3>
        </Link>
